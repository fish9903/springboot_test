...
//    @PostMapping("/add") // 저장 되었다는 알람 띄우기 위해 V4로 이동
    ...
    @PostMapping("/delete")
    public String deletePost(@RequestParam MultiValueMap<String, Object> paramMap, RedirectAttributes redirectAttributes) {
        if(paramMap.size() == 0) {
            return "basic/items";
        }

        List<Object> checkedList = paramMap.get("checked");
